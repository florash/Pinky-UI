...
export function GalleryListMorph({
  ...
  return (
    <div className={cn("w-full", className)} onKeyDown={keyDown}>
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3"><p className="font-mono text-xs text-ink-500">{items.length} items · {effective}</p><div role="group" aria-label="Collection view"><button type="button" aria-pressed={effective === "gallery"} onClick={() => setCurrent("gallery")} className={cn("rounded-l-pill border px-3 py-1.5 text-sm", effective === "gallery" ? "border-ink-900 bg-ink-900 text-milk" : "border-line bg-white text-ink-700")}>Gallery</button><button type="button" aria-pressed={effective === "list"} onClick={() => setCurrent("list")} className={cn("rounded-r-pill border border-l-0 px-3 py-1.5 text-sm", effective === "list" ? "border-ink-900 bg-ink-900 text-milk" : "border-line bg-white text-ink-700")}>List</button></div></div>
