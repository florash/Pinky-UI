...
function ToastViewport({ items, dismiss }: { items: ToastItem[]; dismiss: (id: string) => void }) {
  ...
  return <div aria-label="Notifications" className="pointer-events-none fixed right-4 bottom-[max(1rem,env(safe-area-inset-bottom))] z-[80] flex w-[min(24rem,calc(100vw-2rem))] flex-col gap-2"><AnimatePresence initial={false}>{items.map((item) => <ToastCard key={item.id} item={item} dismiss={dismiss} motionEnabled={motionEnabled} />)}</AnimatePresence></div>;
}
