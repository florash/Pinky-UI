...
export function MorphSelect({ options, value, defaultValue = "", onValueChange, label, placeholder = "Select…", className, disabled = false }: MorphSelectProps) {
  ...
  return (
    <div className={cn("relative", className)}>
      ...
      <motion.button ref={trigger} type="button" disabled={disabled} aria-labelledby={`${id}-label`} aria-haspopup="listbox" aria-expanded={open} onClick={() => { const currentIndex = Math.max(options.findIndex((option) => option.value === selected), 0); setActive(currentIndex); setOpen(true); }} layout={motionEnabled} className="flex w-full items-center justify-between rounded-2xl border border-[color:var(--color-line,rgba(70,90,115,.1))] bg-white px-4 py-3 text-left shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink-900/25 focus-visible:ring-offset-2"><span>{current?.label ?? placeholder}</span><span aria-hidden>⌄</span></motion.button>
      <AnimatePresence>{open ? <motion.div role="listbox" aria-labelledby={`${id}-label`} onKeyDown={keys} initial={motionEnabled ? { opacity: 0, scale: .97, y: -4 } : false} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: .98 }} className="absolute z-40 mt-2 max-h-64 w-full overflow-auto rounded-2xl border border-[color:var(--color-line,rgba(70,90,115,.1))] bg-white p-1.5 shadow-xl">{options.map((option, index) => <button key={option.value} ref={(node) => { nodes.current[index] = node; }} type="button" role="option" aria-selected={option.value === selected} disabled={option.disabled} onClick={() => choose(index)} onMouseMove={() => setActive(index)} className={cn("flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-left text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ink-900/25", active === index && "bg-[color:var(--color-blush-50,#fff7fa)]", option.disabled && "opacity-40")}>{option.label}{option.value === selected ? <span aria-hidden>✓</span> : null}</button>)}</motion.div> : null}</AnimatePresence>
    </div>
  );
}
