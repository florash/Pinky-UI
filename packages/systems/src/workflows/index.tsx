  ...
  return (
    <div aria-label={label} className={cn("w-full max-w-3xl", className)}>
      ...
      <motion.section layout={motionEnabled} aria-labelledby={`${id}-current`} className="mt-3 rounded-[24px] border border-ink-900/10 bg-white p-5 shadow-soft sm:p-6">
        ...
        {current.blocked ? <p className="mt-4 rounded-xl border border-blush-200 bg-blush-50 px-3 py-2 text-xs leading-relaxed text-ink-700">This step stays visible while the product waits; nothing is silently skipped.</p> : null}
    ...
    <motion.div layout={motionEnabled} aria-label={label} className={cn("w-full max-w-4xl rounded-[24px] border border-line bg-white p-4 shadow-soft sm:p-6", className)}>
      <div className="grid gap-3 md:grid-cols-4">
        {stages.map((stage, index) => {
          ...
          return <div key={stage.id} className="relative"><button type="button" onClick={() => index <= activeIndex && setActive(stage.id)} disabled={index > activeIndex} aria-current={stage.id === activeId ? "step" : undefined} aria-label={`${stage.label}, ${state}`} className={cn("relative z-10 flex w-full items-center gap-3 rounded-2xl border p-3 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink-900/20 md:block", state === "current" && "border-ink-900 bg-blush-50", state === "complete" && "border-cloud-300 bg-cloud-50", state === "failed" && "border-ink-900/25 bg-milk", state === "blocked" && "border-blush-200 bg-blush-50", state === "pending" && "border-line bg-cloud-50/60")}><span aria-hidden className={cn("grid size-8 shrink-0 place-items-center rounded-full text-xs md:mb-3", state === "complete" ? "bg-cloud-300 text-ink-900" : state === "failed" ? "bg-ink-900 text-milk" : state === "blocked" ? "bg-blush-200 text-ink-900" : state === "current" ? "bg-ink-900 text-milk" : "bg-white text-ink-400 ring-1 ring-line")}>{state === "complete" ? "✓" : state === "failed" ? "!" : index + 1}</span><span className="min-w-0"><span className="block truncate text-sm font-medium text-ink-900">{stage.label}</span><span className="mt-1 block truncate text-xs text-ink-500">{stage.description}</span><span className="mt-1 block text-[0.625rem] font-medium tracking-[0.08em] text-ink-500 uppercase">{state}</span></span></button>{index < stages.length - 1 ? <span aria-hidden className="absolute top-1/2 right-[-0.75rem] hidden h-px w-3 bg-line md:block" /> : null}</div>;
        })}
      </div>
