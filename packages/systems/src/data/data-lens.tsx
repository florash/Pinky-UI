...
export function DataLens<T>({ items, children, label, index, defaultIndex = 0, onIndexChange, renderLens, className }: DataLensProps<T>) {
  ...
  return <div role="slider" aria-label={label} aria-valuemin={1} aria-valuemax={Math.max(items.length, 1)} aria-valuenow={bounded + 1} tabIndex={0} onPointerMove={seek} onPointerDown={seek} onKeyDown={keys} className={cn("relative touch-pan-y focus-visible:outline-2", className)}>{children}{item ? <div className="pointer-events-none absolute top-3 z-20 -translate-x-1/2 rounded-xl bg-[color:var(--color-ink-900,#252933)] px-3 py-2 text-xs text-white shadow-lg" style={{ left: `${position}%` }}>{renderLens(item, bounded)}</div> : null}<div aria-hidden className="pointer-events-none absolute inset-y-0 w-px bg-[color:var(--color-ink-900,#252933)]/20" style={{ left: `${position}%` }} /></div>;
}
