...
function Demo({ id, title, copy, children, className = "" }: { id: string; title: string; copy: string; children: ReactNode; className?: string }) { return <article id={id} className={`min-w-0 scroll-mt-24 rounded-[28px] border border-line bg-white/75 p-5 shadow-soft sm:p-6 ${className}`}><h3 className="text-xl">{title}</h3><p className="mt-2 mb-6 text-sm leading-relaxed text-ink-700">{copy}</p>{children}</article>; }
